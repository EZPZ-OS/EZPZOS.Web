...
const CoreMenu: React.FC<CoreMenuProps> = ({ isDining }) => {
	...
	return (
		<div className="w-full">
			<div>
				<CoreMenuHorizontalScroll
					categories={currentCategories}
					currentActive={currentActive}
					changeActive={setActiveCategory}
					sortCategories={setCurrentCategories}
				/>
			</div>

			<div className="w-full flex flex-col items-center">
				{Array.isArray(dishes) &&
					dishes.map((dish: CoreMenuDishProps, index: number) => <CoreMenuDishCard key={index} {...dish} />)}
			</div>
		</div>
	);
};
