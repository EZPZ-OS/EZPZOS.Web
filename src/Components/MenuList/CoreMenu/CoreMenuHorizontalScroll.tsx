...
const CoreMenuHorizontalScroll : React.FC<category> = ({categories: currentCategories, currentActive, changeActive, sortCategories})=>{
    ...
    return (
            <div className='w-full  inline-flex h-[48px]'>
                <p className='bg-white  font-bold cursor-pointer h-[48px] leading-[48px] text-16px px-[12px]'>{currentCategories[0]}</p>
                ...
                <div className='overflow-x-auto w-full whitespace-nowrap inline-flex h-[48px] leading-[48px] bg-gray-200 rounded-bl-lg scrollbar-hide'>
                    {currentCategories.slice(1).map((item, index)=>(
                    <span key={index} className={'h-[24px] leading-[24px] text-16px cursor-pointer p-[12px] space-x-[12px]'}
                    onClick={handleClick}>
                        {item}
                    </span>
                ))}
