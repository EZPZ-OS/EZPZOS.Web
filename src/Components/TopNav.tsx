import React ,{ useState }from 'react'
import '../index.css'
import { AiOutlineScan } from "react-icons/ai";
import { FiAlignJustify } from "react-icons/fi";
import { IoSearch } from "react-icons/io5";
import Sidebar from './Sidebar';
/**
 * This is the component of top-navigator,including scanning,searching and sideBar list
 * Sidebar is the child of this components so that topNav can control the visibility of it.
 */


export default function TopNav() {
    //if sideBarVisible is true, sideBar can be seen,otherwise it is invisible.
    const [sideBarVisible,setSidebarVisible] =useState(false)
    const toogleSidebar=()=>{
        setSidebarVisible(!sideBarVisible)
    }
    return (
        <div>
            
            <Sidebar sideBarVisible={sideBarVisible} closeSidebar={toogleSidebar}/>
            <div className='bg-gradient-to-r from-top-nav-lcl to-top-nav-rcl h-[142px] flex'>
                
                {/*AiOutlineScan is the icon of scanning */}
                <div className='w-2/4 text-white mt-14' ><AiOutlineScan className=' text-5xl ml-3'/></div>
                {/*FiAlignJustify is the icon of sideBar list,  
                    IoSearch is the icon of searching
                */}
                <div  className=' w-2/4 flex text-5xl flex-row-reverse text-white mt-14'><FiAlignJustify className='mr-3' onClick={toogleSidebar} style={{display:sideBarVisible?'none':'block'}}/><IoSearch className='mr-3' style={{display:sideBarVisible?'none':'block'}}/></div>
            </div>
            
        </div>
    
    
    
  )
}
