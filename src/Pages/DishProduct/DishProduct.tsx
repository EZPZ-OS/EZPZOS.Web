...
const settings = {
	...
	customPaging: function (i: any) {
		return <span className="w-2 h-2 rounded-circle"></span>;
	}
};
...
const DishProduct = () => {
	...
	const [options, setOptions] = useState([
		{
			Extra: [
				{ title: "Add Sliced Beef", value: true },
				{ title: "Add Stir-fired Egg and Romato", value: false }
			]
		},
	...
	return (
		<div className="relative bg-gradient-to-b from-[#FFFFFF] from-65% to-[#E5D9D3] to-100% pb-5">
			<div className="fixed left-0 top-0 z-10 w-full h-[60px] bg-white">
				<div
					className="absolute left-5 top-5 w-full h-[38px] text-2xl flex flex-row items-center"
					onClick={goBack}
				>
				...
				{authBtn ? (
					<div className="absolute right-5 top-5">
						<span
							className="ml-2 leading-9 bg-orange-500 text-white text-base px-2 rounded"
							onClick={handleEditCuisine}
						>
							Edit
						</span>
						<span
							className="ml-2 leading-9 bg-red-500 text-white text-base px-2 rounded"
			...
			<div className="ml-7 mr-7 mt-[68px]">
				...
				<div className="text-[24px] font-bold mt-5 text-text-dishpro-color [text-shadow:_0_3px_2px_rgb(81_81_81_/_40%)]">
					{dishName}
				</div>
				<ul className="mt-2 flex flex-row">
					<li className="h-5 leading-5 text-sm text-[#5A4F4F] px-2 bg-gradient-to-r from-[#F279A5] to-[#FA986E] opacity-90 rounded-sm mr-1 shadow-tag">
						{category}
					</li>
				</ul>
				<div className="mt-3 text-text-dishpro-color text-xl [text-shadow:_0_3px_2px_rgb(81_81_81_/_40%)]">
					{dishDesc}
				</div>
				<div className="mt-3">
					...
					<div className="mt-1 flex flex-row items-center">
						<span
							className={`w-[112px] h-[32px] leading-8 text-center ${isSpicy ? `bg-red-600 text-white` : `bg-[#C1B3AC] text-[#867777]`} rounded-sm mr-3`}
							onClick={() => setIsSpicy(true)}
						>
							Spicy
						</span>
						<span
							className={`w-[112px] h-[32px] leading-8 text-center ${isSpicy ? `bg-[#C1B3AC] text-[#867777]` : `bg-red-600 text-white`} rounded-sm`}
						...
						<div key={key} className="mt-3">
							...
							<div className="mt-1 text-text-dishpro-color">
								{Object.values(item).map((optionItem, index) => {
									return optionItem.map((optItem: any, optIndex: number) => {
										return (
											<div
												key={optIndex}
												className="h-7 flex flex-row items-center justify-normal"
												onClick={() => handleOptions(key, optIndex)}
											>
												{optItem.value ? (
													<MdOutlineCheckBox />
												) : (
				...
				<div className="mt-3">
					...
					<textarea
						className="w-full h-[50px] mt-1 px-2 shadow-textarea-inset focus:outline-none"
				...
				<div className="w-10/12 mt-3 mx-auto flex flex-row justify-between items-center h-8">
					...
					<div className="w-2/4 h-full flex flex-row items-center justify-around">
						<span
							className="w-8 h-8 leading-8 flex items-center justify-center rounded-circle bg-gradient-to-b from-[#FF993C] to-[#DC4200] opacity-90"
							onClick={() => handProNum("substract")}
						>
							<GrFormSubtract style={{ color: "#fff" }} />
						</span>
						<span className="flex-1 text-center text-text-dishpro-color font-bold text-[20px]">
							{proNum}
						</span>
						<span
							className="w-8 h-8 leading-8 flex items-center justify-center rounded-circle bg-gradient-to-b from-[#FF993C] to-[#DC4200] opacity-90"
							onClick={() => handProNum("add")}
						>
				...
				<div
					className="w-10/12 h-12 leading-[48px] mx-auto bg-gradient-to-r from-[#EF7221] to-[#DC4200] opacity-90 rounded-2xl mt-3 text-center text-white font-bold text-xl"
