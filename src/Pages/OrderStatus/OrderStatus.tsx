...
const OrderStatus = () => {
	...
	return (
		<div className="relative w-full h-screen">
			...
			<div className="w-full leading-[60px] bg-nav-bar-gradient mt-[58px] text-center text-xl text-white font-bold">
				{DefaultOrderStatusPageValues.Welcome}
			</div>
			<div className="flex flex-col justify-center items-center mt-5">
				<img
					src={orderState === 0 ? unreadyImg : orderState === 1 ? preparing : ready}
					className="w-[80px] h-[80px]"
				/>
				<div
					className={`font-bold ${orderState === 0 ? `text-[#988B8B]` : orderState === 1 ? `text-[#FF993C]` : `text-[#65C55D]`} mt-5 text-xl`}
				>
					{orderState === 0
						? DefaultOrderStatusPageValues.NotReady
						: orderState === 1
							? DefaultOrderStatusPageValues.Preparing
							: DefaultOrderStatusPageValues.Ready}
				</div>
			</div>
			<section className="bg-[#F3F3F3] rounded-2xl ml-5 mr-5 mt-7 overflow-hidden">
				<div className="text-center text-[#FB6300] mt-5">
					{DefaultOrderStatusPageValues.DownloadInvoice}{" "}
					<a href="#" className="font-bold">
						{DefaultOrderStatusPageValues.Here}
					</a>
					:)
				</div>
				<h2 className="text-center font-bold text-black text-[24px]">
					{DefaultOrderStatusPageValues.ShopName}
				</h2>
				...
				<div className="pt-5 pb-5 font-bold text-2xl flex justify-center items-center">
					{DefaultOrderStatusPageValues.ThankText}
				</div>
			</section>
		</div>
	);
